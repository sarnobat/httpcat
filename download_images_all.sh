...
cd ~/github/httpcat && groovy -DmaxFailures=5 -DmaxDownloads=${1:-10} download_file_from_list.groovy  /tmp/yurl_queue_httpcat_images_undownloaded.txt /media/sarnobat/3TB/new/images ~/db.git/auto/yurl_queue_httpcat_images_downloaded.txt | tee -a /tmp/yurl_queue_httpcat_images_downloaded_unreliable.txt 2>&1 >/dev/null
